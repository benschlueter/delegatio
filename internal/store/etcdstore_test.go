...
func TestEtcdStore(t *testing.T) {
	...
	// Run the store test.
	//
	store, err := NewEtcdStore([]string{net.JoinHostPort(containerData.NetworkSettings.IPAddress, "2379")}, nil, nil, nil, nil)
	require.NoError(err)
	defer store.Close()
	...
	require.NoError(dockerClient.ContainerStop(ctx, createResp.ID, container.StopOptions{Timeout: nil}))
}
